...
{
	const auto &roster = g.battle.reg.ctx().get<ctx::BattleConfig>().roster;
...
void playStepSounds(Game &g)
{
	...
	for (const sim::CollisionEvent &c : g.battle.collisions())
	{
		...
		for (const sim::EntityId side : {c.a.id, c.b.id})
			if (const auto *w = g.battle.reg.try_get<sim::comp::Warhead>(side);
					w != nullptr)
	...
	{
		// shipIds outlives its ship: the wreck is reaped once it finishes
		// burning, so liveness is asked before the pool is.
		if (!g.battle.alive(shipId))
			continue;
		if (g.battle.reg.all_of<comp::AnnouncedDead>(shipId))
			continue;
		auto *s = g.battle.ship(shipId);
		if (s == nullptr || s->crew > 0)
			continue;
		g.battle.reg.emplace<comp::AnnouncedDead>(shipId);
