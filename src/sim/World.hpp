...
inline constexpr Extent2i kSpace{kScreen.w - kStatusWidth,  // 256
		kScreen.h};                                         // 240
...
constexpr i32 displayToWorld(i32 x) noexcept
{
	return x << kOneShift;
}
constexpr i32 worldToDisplay(i32 x) noexcept
{
	return x >> kOneShift;
}
...
constexpr Vec2i displayToWorld(Vec2i p) noexcept
{
	return Vec2i{displayToWorld(p.x), displayToWorld(p.y)};
}

constexpr Vec2i worldToDisplay(Vec2i p) noexcept
{
	return Vec2i{worldToDisplay(p.x), worldToDisplay(p.y)};
}

constexpr Extent2i displayToWorld(Extent2i e) noexcept
{
...
constexpr i32 wrapX(i32 x) noexcept
{
	if (x < 0)
		return x + kArena.w;
	if (x >= kArena.w)
...
constexpr i32 wrapY(i32 y) noexcept
{
	if (y < 0)
		return y + kArena.h;
	if (y >= kArena.h)
...
constexpr Vec2i wrap(Vec2i p) noexcept
{
...
constexpr i32 wrapDeltaX(i32 dx) noexcept
{
	if (dx < 0)
...
constexpr i32 wrapDeltaY(i32 dy) noexcept
{
	if (dy < 0)
...
constexpr Vec2i wrapDelta(Vec2i d) noexcept
{
