...
| `ElementFlags` bitfield (13 bits, "close to full", review-002) | traits become tag components (`BeamGeometry`, `PlayerShip`, `Cloaked`, `IgnoreVelocity`); per-frame protocol bits stay a small bitfield | the bit-exhaustion problem dissolves; lifecycle bits deliberately do NOT become tags (add/remove churn per frame is noise, not signal) |
...
| X2 | The storage swap: `EntityId` = `entt::entity`, registry + `OrderLink` replace EntityList; `Element` rides whole as one component; Battle's surface unchanged; EntityList/EntityRef deleted | gameplay pins stay **bit-green** (positions, energy, RNG draw order, exactly-once processing — sim_test §5.1-5.4 of the survey); the five tests that pin the *arena itself* are rewritten against the new spine or retired with the commit saying which and why: testEntityAddressesAreStable, testSlotReuseDoesNotReorder, testStaleHandlesAreDetectable (asserts LIFO index reuse and EntityId layout), testTraversalOrderIsWhatWasAskedFor, testRemovalDuringTraversal |
| X3 | Sidecars → pools (`ShipState`, weapon spec); app `Visual` becomes a component, RenderStore and purgeDead deleted | suite green + driven run screenshots |
| X4 | Trait flags → tag components; order-free scans → views | suite green; the flags enum shrinks to the frame-protocol bits |
