# The Ur-Quan Masters — game-logic rewrite plan (`rewrite/core`)
...
**One entity simulation, four games on top of it, a dialogue interpreter, and a bag of screens** —
...
(`ipdisp.c:567`) and — via a fork — the lander (`lander.c:1003-1008`). `init.c:190-198` vs
...
inside `initialize_cone`, calls `SetCustomShipData` → `HFree` + `HMalloc` and rewrites
...
"96.9% expressible" figure does not survive its critique** — its script counted calls to
...
No DSL for AI — `blackurq.c:432`, `chenjesu.c:491` and `zoqfot.c:331` call `ship_weapons` and
...
- **`ShipData` must be constructible in code, not only parsed.** Verified at
  ...
  and then runs the VUX limpet loop `NUM_LIMPETS` times against its own stats — with a comment
...
advertisement (`orz.c:881-885` → 3 lines; makes `chmmr.c:479/664`'s accidental refcount
...
### Worked example — Supox, from `sc2/src/uqm/ships/supox/supox.c` (288 lines)
...
**Estimate for the layer:** 11,413 non-blank lines of `ships/` → roughly 900 lines of ship files
plus ~3,000 lines of C++ (≈15 components, ~19 one-offs, 27 tactics classes). I would defend the
shape firmly and the ratio to ±40%. Three of the ships report's five estimate rows had no stated
...
`:960-1178` DoBuy — which the comm report's extraction ranges skipped — and `:1179-1298` DoSell),
...
- **`GLOBAL_FLAGS_AND_DATA` is not a call parameter.** `commglue.c:391-395` uses bit 7 to decide
  *which race file runs* (`init_spathi_comm` vs `init_spahome_comm`) — consumed before the
...
- **The response columns are gated, not merely ordered.** In `NeutralSupox` every arm of the
  `SUPOX_STACK1` switch also writes `pStr[1] = 0` (`supoxc.c:438-470`), and there is no `case 5` —
  so column 1's topic is unreachable until the player has exhausted `i_am → my_ship →
  from_alliance → are_you_copying → why_copy`. `offer group recent-first` cannot say that.
...
### Worked example — `NeutralSupox`, from `sc2/src/uqm/comm/supox/supoxc.c:307-530`
...
node neutralSupox {
  ...
  on whats_ultron {                              # supoxc.c:383-394 — replaces the offer list
    say TAKE_ULTRON
...
template. `retire`/`fresh` is per-conversation, not saved — `comm.c:1523/1649` load and destroy
...
split-word encodings** the architecture critique found — `MELNORME_CREDIT0/1`,
`RAINBOW_WORLD0/1`, `CREW_PURCHASED0/1`, `CREW_SOLD_TO_DRUUGE0/1` (whose halves are not even
adjacent), `STARBASE_BULLETS0..3` (a 32-bit mask, not a GRPOFFS) — into single values;
transliterating them enshrines the byte-addressing. The generator must also emit an index→field
...
- **Ordered entity list, not an arena.** `disp_q` is spliced into the middle: 20 `InsertElement`
  sites, ~13 at the head. `pkunk.c:503-512` documents why — the phoenix is head-inserted so it
...
- **Pacing.** `ONE_SECOND == 840`, `BATTLE_FRAME_RATE == 840/24 == 35` ticks == 41.67 ms. Use
  ...
  lands on the third 16.7 ms tick — 20 Hz, a 17% global slowdown, browser-only, invisible on
...
- **Sound and HUD are declared outputs of `step()`**, not calls from within it: an event list
  ...
  forcing case — a collision handler that rejection-samples the RNG against a HUD sprite's alpha
...
- **Saves**: one templated `Archive`, chunked as today (`save.c:733-813` — the loader already
  skips unknown chunks). Story chunk is name→value. Save the resume point and the RNG stream
  positions explicitly. Legacy `uqmsave.NN` import, read-only, ~400 lines — kept as a forcing
...
**1. Dialogue transcript oracle — build it before touching `comm/`.** Instrument the current
...
`UQM_DEBUG_COMM` was *added* at `dbd811362` and reverted at `4ac011772` — I verified both commit
...
**2. Content alignment checks in CI, from commit 1.** Preprocessor-aware enum↔`.txt` alignment
...
*every* timestamp for a race behind one warning on a single name mismatch — an entire alien's
...
asteroid field and planet** — `init.c:224-233` puts 5 asteroids and 1 planet in every melee, and
...
timeout rate — nothing in the tree caps battle length), and paired disagreement `d`. **The
critique's fix on the baseline is correct and I adopt it:** perturb *state* (±1 world unit on a
...
sensitivity. Tiers: per-PR 625×24 (catches only catastrophe, which is what per-PR should do);
nightly 625×400 with a BH-ranked delta list; per-milestone 625×2000. **The tournament's job is to
direct human attention, not to grant approval.** Harness self-test: a VUX-vs-anything battle that
terminates — `weapon.c:274-286` rejection-loops until `DrawablesIntersect` succeeds, and
...
`TFB_Random` bit-for-bit (`random.c:61-70` is *not* Park–Miller; it wraps on `DWORD` and returns
...
**M0 — two oracles and a viewer. Start tomorrow morning.** Concrete first hour:
...
3. Capture and commit the Supox transcript set (`supoxc.c` — 5 nodes, 6 foreign statements) as
...
**M1 — vertical slice (the architecture gate).** Human Cruiser vs Ilwrath Avenger, two players at
...
(two different camera *and* sprite-LOD paths, a user option today) and the 320×240 world here or
never. Also in M1, not M5: spike the dialogue model against **Melnorme's `StripShip`** — not for
...
**M2 — Super Melee, 25 ships, front end, AI.** Port `cyborg.c` as literally as C++ allows *first*:
...
Ships in easy→hard order; refuse to generalise a component until its second user exists.

**M3 — hyperspace, starmap, SIS.** Reuses M1's loop, but is 8,389 lines, not 6,000, and it is where
...
**M4 — solar systems, orbit, scan, lander.** 29 generator files, 5,797 lines, spread 671:70 — not
near-clones; they encode hand-placed content across 502 systems.

**M5 — dialogue, adventure state, events, save.** Convert the 21 low-foreign races first, extract
the four systems, then `melnorm`/`starbas`/`chmmrc`/`comandr` against the new verb surface. Delete
all 27 `strings.h`.

**M6 — starbase, outfitting, endgame, cutscenes.** Plus `gameopt.c`/`menu.c`/`flash.c` (~2,900
lines of always-on UI no report scheduled).

**No calendar.** The sequencing report's 35–43 weeks descends from a 25-hour diffstat that includes
...
measuring work of the same kind. Keep `sc2/` building and in CI for the project's whole duration —
...
- **Ship "vtables" as mutable per-instance data.** `chmmr.c:773` deletes its own hook;
  `pkunk.c:282` reinstalls it. These become explicit state machines. Behaviour at the edges —
  which frame the hook stops firing — will differ.
...
- **Match-outcome resolution moves out of ship files.** Three ships own it today —
...
2. **Collision.** Keep swept per-pixel time-of-impact, or move to shapes? I recommend keeping the
   *semantics* and rewriting only the expression — it is the most distinctive and most easily-lost
...
4. **Resolution.** `ScreenWidth`/`ScreenHeight` are runtime globals feeding `LOG_SPACE_WIDTH` and
   `WRAP_X`/`WRAP_Y` — the battlefield's *topology*, not just spawn placement. Decouple the sim's
   coordinate space in M1 or hard-code 320×240 and assert. Not retrofittable.
...
6. **Voice / 3dovoice.** 110 MB, and it replaces six races' `.txt` outright — so it is not an
